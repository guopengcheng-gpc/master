...
public class WebSocketServer {
    ...
    public static void sendMessageAll (String message){
        Set<String> session = webSocketMap.keySet();
        for (String webSocketServer : session) {
            ...
            try {
                if ("".equals(message)){
                    ...
                    map1.put("type","users");
                    map1.put("message",session);
                    map.put("data",map1);
                    message = JSONObject.toJSONString(map1);
                }
                try {
                    webSocketMap.get(webSocketServer).getBasicRemote().sendText(message);
                } catch (IllegalStateException e) {
                    webSocketMap.remove(webSocketServer);
                    sendMessageAll("");
                }
            } catch (Exception e) {
                e.printStackTrace();
